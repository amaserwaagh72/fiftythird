          ...
          {(['all', 'success', 'processing', 'failed', 'canceled'] as const).map(status => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all ${
                filterStatus === status ? 'bg-[#081a44] text-white' : 'bg-white border border-gray-200 text-foreground hover:bg-[#081a44]'
              }`}
            >
        ...
        <div className="banking-card p-4!">
          {filteredTransactions.length === 0 ? (
            ...
            filteredTransactions.map(tx => {
              ...
              return (
                <div key={tx.id} className="flex items-center gap-3 py-3 border-b border-gray-200/50 last:border-0 hover:bg-[#081a44]/10 -mx-2 px-2 rounded-lg transition-colors cursor-pointer">
